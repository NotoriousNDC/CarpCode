...
/// Stateful Server-Sent Events parser. Feed it raw chunks from URLSession.
/// Returns any complete events accumulated since the last call.
///
/// (Lifted near-verbatim from `ios-cowork-prototype/Sources/Core/Utilities/SSEParser.swift`.
/// Kept in sync by intention, not import — keeps the packages independent.)
public struct SSEParser: Sendable {
    ...
    private mutating func extractEvents() -> [SSEEvent] {
        ...
        while let range = buffer.range(of: "\n\n") {
            ...
            if let event = parse(block: block) { events.append(event) }
        }
        return events
    }
